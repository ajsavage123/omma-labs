...
export function PipelineBar({ stages, activeDepartmentStages }: PipelineBarProps) {
  ...
  return (
    <>
      <div className="hidden md:block w-full bg-[#0c0c0e] p-5 md:p-8 md:rounded-[24px] border-y md:border border-white/5 mb-8 md:mb-12 shadow-2xl relative overflow-hidden">
            ...
            {orderedStages.map((name) => {
              ...
              return (
                <div key={name} className="relative z-10 flex flex-col items-center group">
                  ...
                  <span className={`absolute top-12 whitespace-nowrap text-[8px] sm:text-[9px] font-black uppercase tracking-widest transition-colors duration-300 ${
      ...
      {mobileStages.length > 0 && (
        <div className="md:hidden w-full bg-[#0c0c0e]/95 backdrop-blur-xl border-b border-white/5 py-6 mb-8 shadow-xl">
           <div className="px-6 flex items-center justify-between mb-8">
              <span className="text-[10px] font-black uppercase tracking-widest text-[#6366f1]">Department Track</span>
                 ...
                 <div key={stage.id} className={`flex items-center ${isLast ? '' : 'flex-1'}`}>
                    <div className="flex flex-col items-center relative">
                      <div className={`h-8 w-8 rounded-full flex items-center justify-center transition-all ${
                        ...
                        'border-2 border-gray-800 bg-[#11111d] text-gray-700'
                      }`}>
                        {isCompleted ? <Check className="h-4 w-4" strokeWidth={4} /> : 
                         isInProgress ? <div className="h-2 w-2 bg-current rounded-full" /> : null}
                      </div>
                      <span className={`absolute top-10 whitespace-nowrap text-[9px] font-black uppercase tracking-widest ${
                        isCompleted || isInProgress ? 'text-white drop-shadow-md' : 'text-gray-600'
                      }`}>
                    ...
                    {!isLast && (
                      <div className="flex-1 h-[2px] mx-3 bg-gray-800/80 rounded-full overflow-hidden">
                        <div className={`h-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.8)] transition-all duration-1000 ${isCompleted ? 'w-full' : 'w-0'}`}></div>
                      </div>
                    )}
                 </div>
               );
